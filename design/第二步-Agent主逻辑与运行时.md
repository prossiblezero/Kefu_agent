...
4. **调用模型并接收完整决策。** 工具参数完整且通过校验后执行，截断调用不执行。首版按第九步缓存模型正文，收齐校验后发布，前端仅实时展示程序进度事件；物理操作由 `present_troubleshooting_step` 检查后渲染，不直接发布模型自由生成的操作指令。
...
**实现衔接（2026-09-14）：** 自写有界循环与固定确认执行保持不变；调用预算见[第九步](第九步-模型接入与调用管理.md)，任务认领、租约、版本与事件事务见[第十步](第十步-后端接口与持久化.md)，容量与效果仍需实现验证。
