...
**状态（2026-09-12）：五步设计已统一调整为安克全产品售后场景。** 本文为第一步；后续依次为 [运行时](design/第二步-Agent主逻辑与运行时.md)、[工具与 Skill](design/第三步-工具与Skill.md)、[权限](design/第四步-权限与安全边界.md)、[上下文管理](design/第五步-上下文管理.md)。保持原文件名，便于查阅。本轮仅修改设计文档，尚未实现应用。

后续设计：[第六步：Memory](design/第六步-Memory设计调研.md)；[第七步：产品知识库与 RAG](design/第七步-产品知识库与RAG.md)（第一轮草案，讨论知识组织与检索链路）。
...
**文档迁移范围：** 本次修改 Harness 讨论线。`开发执行说明-服务接力-v1.md` 与 `Coding-Agent启动提示词.md` 仍属于旧人工辅助黑客松线，不作为新项目的实施入口；Commerce 学习指南和赛事数据审计保留其原有用途。
...
五份文档已给出本轮可复用的设计依据；技术知识入库、数据集建设与具体前端实现仍属于后续工作。评测分别报告产品消歧、视觉观察、知识适用、排障解决、有效移交、恢复和总成本，不声称已获得实测收益。
